...
TRAIN_CHECK_POINT = 'check_point/ckpt8/'
...
# Get shuffle index
train_video_indices, validation_video_indices = dp.get_video_indices(TRAIN_LIST_PATH)
...
with tf.Graph().as_default():
    ...
    # tf.train.exponential_decay(INITIAL_LEARNING_RATE, global_step, decay_step, LR_DECAY_FACTOR, staircase=True)
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(EPOCH_NUM):
            ...
            for i in range(len(train_video_indices) // BATCH_SIZE):
                step += 1
                batch_data, batch_index = dp.get_batches(TRAIN_LIST_PATH, NUM_CLASSES, batch_index,
            ...
            if (epoch+1) % SAVE_STEP == 0:
                saver.save(sess, TRAIN_CHECK_POINT + 'train.ckpt', global_step=epoch+1)
