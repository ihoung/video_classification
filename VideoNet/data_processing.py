...
np_mean = np.load('crop_mean.npy').reshape([112, 112, 3])
...
def frame_process(clip, crop_size=112, channel_num=3):
    frames_num = len(clip)
    ...
    for i in range(frames_num):
        ...
        if width > height:
            ...
            img = np.array(cv2.resize(img, (int(width * scale + 1), crop_size))).astype(np.float32)
        else:
            scale = float(crop_size) / float(width)
            img = np.array(cv2.resize(img, (crop_size, int(height * scale + 1)))).astype(np.float32)
        crop_x = int((img.shape[0] - crop_size) / 2)
        crop_y = int((img.shape[1] - crop_size) / 2)
        img = img[crop_x: crop_x + crop_size, crop_y: crop_y + crop_size, :]
        croped_frames[i, :, :, :] = img - np_mean
...
def convert_images_to_clip(filename, clip_length=CLIP_LENGTH, crop_size=112, channel_num=3):
    ...
    if len(clip) == 0:
       print(filename)
    clip = frame_process(clip, crop_size, channel_num)
