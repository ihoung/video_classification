...
def _border_expand(image, mode='CONSTANT', constant_values=255,
                   resize=False, output_height=None, output_width=None, channels=3):
...
def _mean_image_subtraction(image, means):
    ...
    for i in range(num_channels):
        channel_mean = tf.convert_to_tensor(means[i])
        shape_list = channels[i].get_shape().as_list()
        channel_mean = tf.reshape(channel_mean, shape=shape_list)
        channels[i] -= channel_mean
